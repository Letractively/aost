...
public class GenericTable extends Container{
...
//  public static final String TAG = "table"
     ...
     private String tagOfTable = "table";
     private String tagOfHead = "thead";
     private String tagOfHeadRow = "tr";
     private String tagOfHeadColumn = "td";
     private String tagOfBody = "tbody";
     private String tagOfBodyRow = "tr";
     private String tagOfBodyColumn = "td";
     private String tagOfFoot = "tfoot";
     private String tagOfFootRow = "tr";
     private String tagOfFootColumn = "td";
...
//                component.cacheable = false
  ...
  public UiObject findUiObject(int tbody, int row, int column) {
    ...
    protected String getCellLocator(int tbody, int row, int column) {
        ...
        return "/${tagOfBody}[${tbody}]/${tagOfBodyRow}[${row}]/${tagOfBodyColumn}[${column}]"
    }

    protected String getCellSelector(int tbody, int row, int column) {

        return " > ${tagOfBody}:eq(${tbody}) > ${tagOfBodyRow}:eq(${row-1}) > ${tagOfBodyColumn}:eq(${column-1})"
    }

    protected String getHeaderLocator(int column) {

        return "/${tagOfHead}/${tagOfHeadRow}/${tagOfHeadColumn}[${column}]"
    }

    protected String getHeaderSelector(int column) {

        return " > ${tagOfHead} > ${tagOfHeadRow} > ${tagOfHeadColumn}:eq(${column-1})"
    }

    protected String getFootLocator(int column) {

        return "/${tagOfFoot}/${tagOfFootRow}/${tagOfFootColumn}[${column}]"
    }

    protected String getFootSelector(int column) {

        return " > ${tagOfFoot} > ${tagOfFootRow} > ${tagOfFootColumn}:eq(${column-1})"
    }

    String[] getAllTableCellText(Closure c) {
        return c(this.locator, " > ${tagOfBody} > ${tagOfBodyRow} > ${tagOfBodyColumn}")
    }

    String[] getAllTableCellTextForTbody(int index, Closure c) {
        return c(this.locator, " > ${tagOfBody}:eq(${index-1}) > ${tagOfBodyRow} > ${tagOfBodyColumn}")
    }
    ...
    int getTableHeaderColumnNumBySelector(Closure c) {
        return c(this.locator, " > ${tagOfHead} > ${tagOfHeadRow}:eq(0) > ${tagOfHeadColumn}")
    }

    int getTableFootColumnNumBySelector(Closure c) {
        return c(this.locator, " > ${tagOfFoot} > ${tagOfFootRow}:eq(0) > ${tagOfFootColumn}")
    }

    int getTableMaxRowNumBySelector(Closure c) {
        return c(this.locator, " > ${tagOfBody}:eq(0) > ${tagOfBodyRow}:has(td)")
    }

    int getTableMaxRowNumForTbodyBySelector(int ntbody, Closure c) {
        return c(this.locator, " > ${tagOfBody}:eq(${ntbody-1}) > ${tagOfBodyRow}:has(td)")
    }

    int getTableMaxColumnNumBySelector(Closure c) {
        return c(this.locator, " >  ${tagOfBody}:eq(0) > ${tagOfBodyRow}:eq(0) > ${tagOfBodyColumn}")
    }

    int getTableMaxColumnNumForTbodyBySelector(int ntbody, Closure c) {
         return c(this.locator, " > ${tagOfBody}:eq(${ntbody-1}) > ${tagOfBodyRow}:eq(0) > ${tagOfBodyColumn}")
    }

    int getTableMaxTbodyNumBySelector(Closure c){
         return c(this.locator, " > ${tagOfBody}")
    }
...
//          loc = getCellSelector(ntbody-1, nrow-1, ncolumn-1)
 ...
 //             if("td".equals(cl.tag) && cl.header == null){
              if(cl.isInside() || ("td".equals(this.tagOfBodyColumn) && this.tagOfBodyColumn.equals(cl.tag) && cl.header == null)){
    ...
    protected walkToHeader(WorkflowContext context, UiID uiid) {
        ...
        if(cobj.locator != null){
          if(cobj.locator instanceof CompositeLocator){
            CompositeLocator cl = (CompositeLocator)cobj.locator
 //           if("td".equals(cl.tag) && cl.header == null){
            if(cl.isInside() || ("td".equals(this.tagOfHeadColumn) && this.tagOfHeadColumn.equals(cl.tag) && cl.header == null)){
              //context.setTableDuplicateTag()
              context.skipNext()
            }
          }
    ...
    protected walkToFoot(WorkflowContext context, UiID uiid) {
        ...
        if(cobj.locator != null){
          if(cobj.locator instanceof CompositeLocator){
            CompositeLocator cl = (CompositeLocator)cobj.locator
 //           if("td".equals(cl.tag) && cl.header == null){
            if(cl.isInside() || ("td".equals(this.tagOfFootColumn) && this.tagOfFootColumn.equals(cl.tag) && cl.header == null)){
              //context.setTableDuplicateTag()
              context.skipNext()
            }
          }
